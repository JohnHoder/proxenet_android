 ...
 *
 */
extern int proxenet_yolo(int argc, char **argv, char **envp)
{
        int retcode = -1;
        (void) *envp;
...
void* print_xs (const char* text)
{
        LOGI("%s", text);
}

void Java_janhodermarsky_proxenet_ProxenetService_fireProxenet(JNIEnv *env, jobject thiz)
{
    ALOGV("Hello from Proxenet NDK layer!");
    //showBread("Callback from single native thread done.\n", strlen("Callback from single native thread done.\n"));
    threadShowBread_start("Callback from single native thread done.\n");
