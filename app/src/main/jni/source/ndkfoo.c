...
void Java_janhodermarsky_proxenet_ProxenetService_initJNICallback(JNIEnv* env, jobject thiz) {

        g_javaObj = (*env)->NewGlobalRef(env, thiz);
        cachedContext = thiz;

        int gotVM = (*env)->GetJavaVM(env, &cachedJVM);
...
void *showBreadx(const char* message)
{
    JNIEnv env = NULL;
    env = getJNIEnv();
    ...
    //(*g_javaObj)->AttachCurrentThread(g_javaObj, &env, NULL);
    if(message == NULL){
        return;
    }
        ...
        if (!getMethodInfo(&methodInfo, "makeText", "(Landroid/content/Context;Ljava/lang/CharSequence;)Ljava/lang/String;"))    //"(Landroid/content/Context;Ljava/lang/CharSequence;)Ljava/lang/String;"  vs. "(Landroid/content/Context;[B)Ljava/lang/String;"
        {
            ...
            LOGD("Method makeText was found! \\o/");
        }

        jstring jstr = (*methodInfo.env)->NewStringUTF(methodInfo.env, message);
        //*************************************************//
        ...
        //(*methodInfo.env)->ReleaseByteArrayElements(methodInfo.env, array, message, JNI_ABORT);
        //(*methodInfo.env)->DeleteLocalRef(methodInfo.env, array);
        /////
    ...
    //LOGI("showBread finished.");
}

void threadShowBread_start(const char message){
    ...
    if(ret = pthread_create(&thread, NULL, &showBreadx, message) == 0)
    {
